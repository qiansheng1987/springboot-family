...
public abstract class AbstractExcelImportController<ExcelImportBeanType> {
     ...
     * 本地临时文件存放地址
     */
    private static final String TEMP_FILL_PATH =
     ...
     * 上传人
     */
    protected ThreadLocal<EmployeeInfo> uploader = new ThreadLocal<>();
    /**
     * The Workbook.
     */
    ...
    private void initLocaleContext() {
        ...
        //uploader.set(SpringUtil.getBean("userInfo"));
    }
     ...
     */
    private File uploadTempFile(byte[] itemBytes, String fileName) throws IOException {
        String fileFullPath = TEMP_FILL_PATH + fileName;
    ...
    private File uploadFile(MultipartHttpServletRequest request) throws IOException {
        ...
        String fileName = cn.hutool.core.date.DateUtil.format(new Date(), "yyyyMMddHHmmssSSS") + multipartFile.getOriginalFilename();
        @SuppressWarnings("UnnecessaryLocalVariable") File excelFile = uploadTempFile(itemBytes, fileName);
    ...
    private void bindRowToBean(Row row, int startIndex, int endIndex) {
        ...
        try {
            // 新建数据实例
            ExcelImportBeanType bean = this.bindClass.newInstance();
    ...
    private Boolean bind(String value, Field field, Object bean) throws RuntimeException {
        ...
        if (this.bindMethodMap.containsKey(fieldType)) {
            ...
            try {
                ...
                if (ObjectUtil.isNotNull(e.getCause()) && !StringUtils.isBlank(e.getCause().getMessage())) {
